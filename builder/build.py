...
if ICON_FILE.exists():
    try:
        ...
        for folder, size in MIPMAP_SIZES.items():
            d = WORK_DIR / "res" / folder
            d.mkdir(parents=True, exist_ok=True)
            img.resize((size, size), Image.LANCZOS).save(d / "ic_launcher.png")
        print("✅ Icon processed")
    except Exception as e:
        print(f"⚠️  Icon processing failed: {e} — using default")
        ICON_FILE = None
else:
    print("ℹ️  No icon provided — using default Android launcher icon")

# If no custom icon, create a simple default colored square
if not ICON_FILE or not ICON_FILE.exists():
    try:
        ...
        # Absolute fallback — just make the dirs so aapt2 doesn't error
